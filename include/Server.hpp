...
class Server
{
	...
	private:
		...
		int				sendResponse(int client_fd, const std::string& status, const std::string& content, bool& chunked);
		...
		int				checkLocations(std::vector<Location>::iterator& location_it, std::vector<Location>::iterator& location_ite, std::string& file_path, std::string source, int client_fd, bool& chunked, epoll_event* event);
